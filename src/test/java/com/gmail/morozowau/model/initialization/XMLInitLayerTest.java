...
class XMLInitLayerTest {
    private List<Address> expected = new ArrayList<>();
    private String testPath = "src/test/resources/com/gmail/morozowau/model/initialization/XMLInitTest/Addresses.xml";
    private final int numberOfTests = 1_000;
    private XMLInitLayer csvInitLayer = new XMLInitLayer(testPath);

    {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(testPath))) {
            for (int i = 0; i < numberOfTests; i++) {
                ...
                String initData = String.format("<item city=\"%s\" street=\"%s\" house=\"%d\" floor=\"%d\" />",
                        city,
                        street,
                        house,
                        floor);
    ...
    @Test
    void WhenParsingStringThenDataTakeFromItems() throws IOException {
